...
func (s *endpointCaptureDB) UpsertNode(context.Context, UpsertNodeParams, RadioSettings) (uuid.UUID, bool, error) {
	s.missingNode = false
	return s.node.ID, false, nil
}
...
func (s *endpointCaptureDB) ResolvePathHashes(_ context.Context, hashes [][]byte) (map[string][]api.ResolvedPathEntry, error) {
	if len(hashes) > 0 {
		s.pathLookups++
	}
