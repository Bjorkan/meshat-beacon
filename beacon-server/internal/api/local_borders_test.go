...
func (r *borderReader) ListNodes(_ context.Context, _ api.NodeListParams) (api.Page[api.NodeSummary], error) {
	r.listCalls++
	return r.page, r.err
}
...
func TestLocalBordersAfterCache(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		page, err := tc.reader.ListNodes(ctx, api.NodeListParams{NodeType: 2, IATAs: []string{"ZZZ"}, Limit: 50})
		if err != nil {
			t.Fatal(err)
		}
...
func TestLocalBordersPreserveReaderErrors(t *testing.T) {
	...
	if _, err := reader.ListNodes(context.Background(), api.NodeListParams{Limit: 1}); !errors.Is(err, want) {
		t.Fatal(err)
	}
	base.err = nil
