...
INSERT INTO trace_iatas (trace_tag,iata) VALUES
 ('\xaaaaaaaa','YYZ'),('\xaaaaaaaa','YVR'),('\xbbbbbbbb','YYZ'),('\xcccccccc','YVR');`)
	...
	summary := func(tag string, first, last int, count, iatas int64, kind string) api.TraceTagSummary {
		resolved := make([]api.ResolvedHopLite, len(paths[tag]))
		for i := range resolved {
			resolved[i].Confidence = "none"
		}
		return api.TraceTagSummary{ResolvedPath: resolved, TraceTag: strings.Repeat(tag, 8), FirstHeardAt: at(first).UnixMilli(), LastHeardAt: at(last).UnixMilli(), PacketCount: count, IATACount: iatas, TraceType: kind, PathHashes: paths[tag], SNRValues: snrs[tag]}
	}
