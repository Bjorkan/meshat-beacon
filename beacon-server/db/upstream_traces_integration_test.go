...
ANALYZE packets; ANALYZE packet_observations; ANALYZE nodes; ANALYZE node_short_ids;`)
	...
	for i, p := range detail.Packets {
		...
		// Meshat resolves relays globally: both regional candidates remain ambiguous,
		// including packets that do not have an observation yet.
		if len(p.ResolvedRoute) != 1 || p.ResolvedRoute[0].Confidence != "ambiguous" || len(p.ResolvedRoute[0].Nodes) != 2 {
			t.Fatalf("global collision was hidden at %d: %+v", number, p.ResolvedRoute)
		}
