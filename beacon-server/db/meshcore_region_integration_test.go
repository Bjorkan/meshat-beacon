...
func TestMeshCoreRegionIntegration(t *testing.T) {
	...
	if err := store.UpsertNodeNeighbor(ctx, alpha, bravo, "YVR", nil, strPtr("se"), true, i16(32)); err != nil {
		t.Fatal(err)
	}
	// A later timeout (NULL scope) must preserve bravo's value AND timestamp.
	if err := store.UpsertNodeNeighbor(ctx, alpha, bravo, "YVR", nil, nil, false, nil); err != nil {
		t.Fatal(err)
	}
