...
describe('parseEnvBool', () => {
  ...
  it('is false for unset, empty, sentinel, or other values', () => {
    for (const v of [undefined, '', '0', 'false', 'no', '__VITE_SKIP_SPLASH__'])
...
describe('selectableThemes', () => {
  ...
  it('shows ONLY the allowlisted themes when set (exclusive), including hidden ones', () => {
    const ids = selectableThemes(THEMES, new Set(['meshmapper_dark', 'meshmapper_light'])).map(
      (t) => t.id,
    );
  ...
  it('matches allowlist ids case-insensitively', () => {
    ...
    expect(selectableThemes(themes, new Set(['meshmapper_dark'])).map((t) => t.id)).toEqual([
      'MeshMapper_Dark',
    ]);
  });
});
