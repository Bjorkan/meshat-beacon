...
describe('application routes', () => {
  ...
  it('validates map state at the route boundary', async () => {
    const router = await routerAt(
      '/map?lat=59.33&lng=18.07&zoom=9&clustering=off&node_type=repeater&flow=on',
    );
  ...
  it('validates Map and Analytics state at their own route boundaries', async () => {
    ...
    expect(routeSearch(analyticsRouter)).toMatchObject({
      statsTab: 'observer',
      observerId: 'o1',
      range: '7d',
    });
  ...
  it('validates Nodes, Observers, Channels, and Traces list state from URLs', async () => {
    const nodes = await routerAt(
      '/nodes?nq=alice&nsf=pubkey&nt=REPEATER&np=true&ntr=false&ns=%23east&nsort=radio&ndir=desc',
    );
    ...
    expect(routeSearch(channels)).toMatchObject({
      cq: 'ops',
      csf: 'hash',
      ck: 'known',
      ch: 'false',
  ...
  it('falls back safely for invalid list params', async () => {
    const nodes = await routerAt(
      '/nodes?nsf=anything&np=maybe&ntr=yes&nsort=last_seen&ndir=sideways',
    );
    expect(routeSearch(nodes)).toMatchObject({
      nsf: undefined,
      np: undefined,
