...
import {
  ...
  trailCoords,
  pathWithinLoRaRange,
} from '../../../src/features/map/packet-flow';
...
describe('packetChain', () => {
  ...
  it('drops ambiguous and unresolved endpoints', () => {
    const ambiguous: ResolvedHop = {
      ...
      nodes: [{ id: 'x', publicKey: 'pk', longitude: -70, latitude: 40 }],
    };
  ...
  it('accepts null or absent endpoints and leaves relay hops untouched', () => {
    const ambiguousRelay: ResolvedHop = {
      confidence: 'ambiguous',
      nodes: [{ id: 'y', publicKey: 'pk', longitude: -76, latitude: 46 }],
    };
...
describe('resolvedPathNodes', () => {
  ...
  it('skips hops with no located candidate', () => {
    expect(
      resolvedPathNodes([{ confidence: 'ambiguous', nodes: [{ id: 'x', publicKey: 'pk' }] }]),
    ).toEqual([]);
  });
...
describe('trailCoords', () => {
  ...
  it('traces every crossed hop plus the current head position', () => {
    expect(trailCoords(coords, 1.5)).toEqual([
      [0, 0],
      [10, 0],
      [10, 5],
    ]);
