...
describe('patchNodeSummary', () => {
  ...
  it('patches name/lat/lng of the matching node immutably', () => {
    ...
    const out = patchNodeSummary(
      list,
      update({ nodeId: 'b', name: 'Renamed', lat: 50, lng: -80 }),
    )!;
...
describe('Nodes-table live update policy', () => {
  it('patches summary fields that are safe to update in place', () => {
    ...
    const out = patchNodeTableSummary(
      list,
      update({
        nodeId: 'a',
        name: 'Keep',
  ...
  it('requires a refetch when the active server sort key changes', () => {
    ...
    expect(nodeListUpdateRequiresRefetch(prev, update({ name: 'Zulu' }), { sort: 'name' })).toBe(
      true,
    );
    expect(
      nodeListUpdateRequiresRefetch(prev, update({ name: 'Zulu' }), { sort: 'neighbors' }),
    ).toBe(false);
  });
  ...
  it('requires a refetch only when active filter membership changes', () => {
    ...
    expect(
      nodeListUpdateRequiresRefetch(prev, update({ name: 'Zulu' }), {
        sort: 'neighbors',
        name: 'alp',
      }),
    ).toBe(true);
    expect(
      nodeListUpdateRequiresRefetch(prev, update({ name: 'Alpha', defaultScope: '#west' }), {
        sort: 'neighbors',
        scope: '#east',
      }),
    ).toBe(true);
    expect(
      nodeListUpdateRequiresRefetch(
        prev,
        update({ name: 'Alpha', iatas: [{ iata: 'YVR', lastHeard: 2 }] }),
        { sort: 'neighbors', iatas: ['YOW'] },
      ),
    ).toBe(true);
    expect(
      nodeListUpdateRequiresRefetch(prev, update({ name: 'Alpha', lat: 46 }), {
        sort: 'neighbors',
        name: 'alp',
      }),
    ).toBe(false);
...
describe('upsertNodePages', () => {
  ...
  it('keeps the same ref for a re-advert that changes nothing map-visible', () => {
    ...
    expect(
      upsertNodePages(old, update({ nodeId: 'a', name: 'Keep', lat: undefined, lng: undefined })),
    ).toBe(old);
  });
