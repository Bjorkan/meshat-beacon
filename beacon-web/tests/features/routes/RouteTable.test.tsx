...
describe('RouteTable search', () => {
  ...
  it('searches cross-IATA when two IATAs are selected, rendering the segmented chain', async () => {
    const cross: CrossIATARoute = {
      ...
      crossHop: {
        fromNode: node('n1', 'Src Node'),
        toNode: node('n2', 'Dst Node'),
        fromIata: 'AAA',
        toIata: 'BBB',
        lastSeen: 2,
      },
  ...
  it('sends every selected region IATA to the backend', async () => {
    ...
    mockGetKnownRoutesPage.mockResolvedValue({
      items: [wanted],
      nextPageToken: null,
      nextCursor: null,
      hasMore: false,
    ...
    expect(mockGetKnownRoutesPage).toHaveBeenCalledWith(
