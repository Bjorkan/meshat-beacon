...
const observer = (
  ...
  return {
    id: 'abcdef1234',
    displayName: 'Cypress Peak',
    iata: 'YVR',
    pathLength: { raw: '1e', hashSize, hopCount },
    ...rest,
  };
};
...
describe('PacketTableRow', () => {
  ...
  it('shows observer identity and area inline', () => {
    renderRow(
      <PacketTableRow
        packet={pkt({ latestObserver: observer() })}
        expanded={false}
        onToggle={() => {}}
      />,
    );
  ...
  it('falls back to observer id prefix when displayName is absent', () => {
    renderRow(
      <PacketTableRow
        packet={pkt({ latestObserver: observer({ displayName: undefined }) })}
        expanded={false}
        onToggle={() => {}}
      />,
    );
  ...
  it('combines hop count and hash size into one compact diagnostic', () => {
    renderRow(
      <PacketTableRow
        packet={pkt({ latestObserver: observer({ hopCount: 5, hashSize: 3 }) })}
        expanded={false}
        onToggle={() => {}}
      />,
    );
  ...
  it('shows raw inline path without expanding', () => {
    renderRow(
      <PacketTableRow
        packet={pkt({ latestObserver: observer({ pathBytes: '7fa4' }) })}
        expanded={false}
        onToggle={() => {}}
      />,
    );
  ...
  it('keeps one compact desktop row and shows n/a when observer data is absent', () => {
    const { container } = renderRow(
      <PacketTableRow packet={pkt()} expanded={false} onToggle={() => {}} />,
    );
