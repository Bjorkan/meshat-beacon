...
describe('buildPathSummary', () => {
  ...
  it('labels a literal zero-hop path as direct', () => {
    expect(
      buildPathSummary(pkt(obs({ pathLength: { raw: '00', hashSize: 1, hopCount: 0 } }))),
    ).toMatchObject({
      hopLabel: '0 hops · direct',
      chips: [],
      isNa: false,
    });
  });
  ...
  it('keeps the raw hash primary for ambiguous resolution and exposes candidate count', () => {
    ...
    expect(s.chips).toEqual([
      { kind: 'hex', label: '7F', confidence: 'ambiguous', candidateCount: 2 },
    ]);
  });
