...
import {
  ...
  toServerFilter,
  usePacketFilters,
} from '../../../src/features/packets/usePacketFilters';
...
describe('toServerFilter', () => {
  ...
  it('pushes a multi-value payload-type selection server-side', () => {
    expect(
      toServerFilter({ ...EMPTY_FILTERS, payloadTypes: [2, 4] as PayloadTypeValue[] }),
    ).toEqual({ payloadTypes: [2, 4] });
  });

  it('emits payloadTypes for a single selected type', () => {
    expect(toServerFilter({ ...EMPTY_FILTERS, payloadTypes: [4] as PayloadTypeValue[] })).toEqual({
      payloadTypes: [4],
    });
  });

  it('emits routeTypes including 0 (falsy) for a selected route', () => {
    expect(toServerFilter({ ...EMPTY_FILTERS, routeTypes: [0] as RouteTypeValue[] })).toEqual({
      routeTypes: [0],
    });
  });

  it('emits scopes for selected scopes', () => {
    expect(toServerFilter({ ...EMPTY_FILTERS, scopes: ['#bc', '#west'] })).toEqual({
      scopes: ['#bc', '#west'],
    });
  });
  ...
  it('emits every selected dimension together', () => {
    ...
    expect(toServerFilter(filters)).toEqual({
      payloadTypes: [4],
      routeTypes: [1, 2],
      scopes: ['#bc'],
    });
