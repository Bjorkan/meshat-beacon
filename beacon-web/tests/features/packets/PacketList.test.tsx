...
vi.mock('../../../src/features/packets/PacketVirtualList', () => ({
  PacketVirtualList: ({
    ...
    <div>
      ...
      <button type="button" onClick={onOpenAnalyzer}>
        Open analyzer
      </button>
      <button type="button" onClick={onViewPath}>
        View path on map
      </button>
      {packets.map((p) => (
        <button
      ...
      data: {
        ...
        observations: [
          ...
          },
          {
            ...
            iata: 'MMX',
            heardAt: 1700000002,
            pathLength: { raw: '11', hashSize: 1, hopCount: 1 },
            pathBytes: 'AB',
            sourceBroker: 'b1',
            resolvedPath: [
              { confidence: 'high', nodes: [{ id: 'n1', name: 'Node AB', publicKey: 'ab' }] },
            ],
          },
        ],
      },
