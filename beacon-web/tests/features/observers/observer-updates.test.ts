...
describe('patchObserverSummary', () => {
  ...
  it('patches the complete live summary of the matching observer immutably', () => {
    ...
    const out = patchObserverSummary(
      list,
      update({
        observerId: 'b',
        online: true,
  ...
  it('returns the same list ref when the update changes nothing', () => {
    ...
    const out = patchObserverSummary(
      list,
      update({ observerId: 'a', online: true, displayName: 'Keep', lastStatusAt: 1000 }),
    );
  ...
  it('updates lastStatusAt even when the sort/filter fields are unchanged', () => {
    ...
    const out = patchObserverSummary(
      list,
      update({ observerId: 'a', online: true, displayName: 'Keep', lastStatusAt: 200 }),
    )!;
    expect(out).not.toBe(list);
...
describe('Observers-table live update policy', () => {
  it('refetches when a server ordering key changes', () => {
    ...
    expect(
      observerListUpdateRequiresRefetch(prev, update({ displayName: 'Zulu', online: false }), {
        sort: 'name',
      }),
    ).toBe(true);
    expect(
      observerListUpdateRequiresRefetch(prev, update({ displayName: 'Alpha', online: true }), {
        sort: 'status',
      }),
    ).toBe(true);
    expect(
      observerListUpdateRequiresRefetch(
        prev,
        update({ displayName: 'Alpha', online: false, lastStatusAt: 2000 }),
        { sort: 'name' },
      ),
    ).toBe(false);
  });
  ...
  it('refetches when active server filter membership changes', () => {
    ...
    expect(
      observerListUpdateRequiresRefetch(
        prev,
        update({ displayName: 'Alpha', online: false, scopes: ['#west'] }),
        { sort: 'last_seen', scope: '#east' },
      ),
    ).toBe(true);
    expect(
      observerListUpdateRequiresRefetch(
        prev,
        update({ displayName: 'Alpha', online: false, iata: 'YVR' }),
        { sort: 'last_seen', iatas: ['YOW'] },
      ),
    ).toBe(true);
    expect(
      observerListUpdateRequiresRefetch(
