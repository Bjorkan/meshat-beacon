...
import {
  trafficModel,
  trafficHeatmapOption,
  trafficTrendOption,
} from '../../../src/features/stats/traffic';
...
const point = (time: number, iata: string, count: number): ObservationPoint => ({
  hour: time,
  iata,
...
describe('traffic exploration', () => {
  it('counts receptions, keeps missing hours null, and anchors the bounded window in UTC', () => {
    const data = [
      point(end - 2 * hour, 'YVR', 2),
      point(end - 2 * hour, 'YOW', 3),
      point(end, 'YVR', 7),
    ];
    ...
    expect(model.areas.map((area) => [area.name, area.total])).toEqual([
      ['YVR', 9],
      ['YOW', 3],
    ]);
  ...
  it('handles empty data without invented traffic and exposes safe, gap-preserving chart options', () => {
    ...
    expect(trafficHeatmapOption(populated, colors)).toMatchObject({
      animation: false,
      tooltip: { renderMode: 'richText' },
      visualMap: { min: 0, max: 1 },
    });
    expect(trafficTrendOption(populated, colors)).toMatchObject({
      animation: false,
      tooltip: { renderMode: 'richText' },
