...
const fixture: PathStats = {
  ...
  unclassified: 1,
  hashWidths: [
    { bytes: 1, receptions: 2 },
    { bytes: 2, receptions: 3 },
    { bytes: 3, receptions: 1 },
  ],
  pathLengths: [
    { entries: 0, receptions: 2 },
    { entries: 2, receptions: 5 },
    { entries: 63, receptions: 1 },
  ],
  hourly: [
    {
      hour: 0,
...
describe('path analytics charts', () => {
  ...
  it('fills zero-count length bins while preserving empty-path counts and visible single-hour points', () => {
    ...
    expect(pathTrendOption(pathHours(fixture), c)).toMatchObject({
      animation: false,
      useUTC: true,
      legend: { top: 0 },
      series: [
