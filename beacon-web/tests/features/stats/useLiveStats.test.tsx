...
describe('useLiveObserver', () => {
  it("refreshes the selected observer's telemetry, but not its heard activity, on a status event", () => {
    const qc = new QueryClient();
    qc.setQueryData(['observer-telemetry', 'obs-1', '24h', '1h'], {
      range: '24h',
      interval: '1h',
      points: [],
    });
    qc.setQueryData(['observer-activity', 'obs-1', '24h'], {
      range: '24h',
      interval: '15m',
      radio: null,
      payloadTypes: [],
    ...
    expect(qc.getQueryState(['observer-telemetry', 'obs-1', '24h', '1h'])?.isInvalidated).toBe(
      true,
    );
