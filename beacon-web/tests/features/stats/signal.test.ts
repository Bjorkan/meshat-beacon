...
import {
  ...
  signalTrendOption,
} from '../../../src/features/stats/signal';
...
export const signalFixture: SignalStats = {
  ...
  snr: {
    ...
    average: 0,
    histogram: [
      { lower: null, upper: -30, count: 2 },
      { lower: -30, upper: -25, count: 78 },
      { lower: 30, upper: null, count: 0 },
    ],
  },
...
describe('signal analytics', () => {
  it('keeps UTC window edges, true zeros, missing hours and missing readings distinct', () => {
    ...
    expect(
      signalHours({ ...signalFixture, until: 30 * 24 * hour + hour / 2, hourly: [] }),
    ).toHaveLength(721);
  });
  it('labels exact half-open bins including overflow without assigning quality ratings', () => {
    ...
    expect(signalHistogramOption(signalFixture.snr, 'SNR', 'dB', colors)).toMatchObject({
      animation: false,
      tooltip: { renderMode: 'richText' },
      aria: { enabled: true },
    });
    expect(signalTrendOption(signalHours(signalFixture), 'snr', colors)).toMatchObject({
      animation: false,
      series: [
        {
          connectNulls: false,
