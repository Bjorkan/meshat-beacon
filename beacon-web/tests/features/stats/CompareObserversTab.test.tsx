...
function mount(url = '?statsTab=compare') {
  ...
  return render(
    <QueryClientProvider client={client}>
      <TestRouter routePath="/analytics" initialEntry={`/analytics${url}`}>
        <CompareObserversTab />
        <Location />
      </TestRouter>
    </QueryClientProvider>,
...
beforeEach(() => {
  ...
  vi.mocked(getObserversPage).mockResolvedValue({
    items: [
  ...
  vi.mocked(getObserver).mockImplementation(async (id) => ({
    id,
    displayName: id === a ? 'Rooftop' : 'Hilltop',
    iata: 'YVR',
    status: 'online',
    publicKey: '00',
    firstSeen: 0,
...
describe('observer comparison', () => {
  it('waits for Compare, sends explicit dates and region, and puts the selection in the URL', async () => {
    ...
    await waitFor(() =>
      expect(getObserverComparison).toHaveBeenCalledWith(
        ['YVR'],
        {
          observerA: a,
          observerB: b,
          since: new Date('2026-01-01T00:00').getTime(),
          until: new Date('2026-02-15T00:00').getTime(),
        },
  ...
  it('renders an empty period without NaN percentages', async () => {
    vi.mocked(getObserverComparison).mockResolvedValue({
      ...counts,
      totalPackets: 0,
      onlyA: 0,
      onlyB: 0,
  ...
  it('searches the server beyond the initial observer page', async () => {
    mount();
    fireEvent.change(await screen.findByLabelText('Search Observer A'), {
      target: { value: 'Distant station' },
    });
    await waitFor(() =>
      expect(getObserversPage).toHaveBeenCalledWith(['YVR'], {
  ...
  it('rejects malformed shared dates without issuing a comparison', async () => {
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent(/invalid or missing/);
    await screen.findAllByRole('option', { name: /Rooftop/ });
    expect(getObserverComparison).not.toHaveBeenCalled();
  });

  it('shows query failures and retries only when requested', async () => {
    vi.mocked(getObserverComparison).mockRejectedValueOnce(
      new Error('comparison timed out; try a shorter time period'),
    );
    mount(query);
