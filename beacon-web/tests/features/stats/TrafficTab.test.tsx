...
const query = {
  ...
  dataUpdatedAt: Date.UTC(2026, 8, 19, 12, 30),
  isLoading: false,
...
vi.mock('../../../src/features/stats/useStats', () => ({
  useStatsObservations: vi.fn(() => query),
}));
...
describe('Traffic page', () => {
  it('shows exact reception counts, UTC/missing-history guidance and a refresh action', () => {
    ...
    expect(within(table).getByRole('row', { name: /YOW.*1[\s,]200.*100.0%/ })).toBeInTheDocument();
