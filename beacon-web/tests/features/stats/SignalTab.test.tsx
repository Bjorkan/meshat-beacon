...
const query = {
  data: {
    ...
    until: 3600000,
    receptions: 100,
    snr: { samples: 80, average: 0, histogram: [{ lower: -5, upper: 0, count: 80 }] },
...
vi.mock('../../../src/features/stats/useSignalStats', () => ({
  useSignalStats: vi.fn(() => query),
}));
...
it.each(['isPending', 'isPlaceholderData', 'isError'] as const)(
  'hides previous filter values when %s',
  (state) => {
    query[state] = true;
