...
import {
  filterChannels,
  type ChannelFilters,
} from '../../../src/features/channels/channel-filters';
...
describe('filterChannels', () => {
  ...
  it('filters by key known / unknown', () => {
    expect(filterChannels(channels, { ...NONE, keyFilter: 'known' }).map((c) => c.id)).toEqual([
      1, 2,
    ]);
    expect(filterChannels(channels, { ...NONE, keyFilter: 'unknown' }).map((c) => c.id)).toEqual([
      3,
    ]);
  });

  it('filters by hashtag', () => {
    expect(filterChannels(channels, { ...NONE, hashtagFilter: 'true' }).map((c) => c.id)).toEqual([
      2,
    ]);
    expect(filterChannels(channels, { ...NONE, hashtagFilter: 'false' }).map((c) => c.id)).toEqual([
      1, 3,
    ]);
  });
