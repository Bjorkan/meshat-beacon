...
function observerStatus(
  overrides: Partial<WsObserverStatus['data']> = {},
): WsObserverStatus['data'] {
  return {
...
describe('global WebSocket to Query cache policy', () => {
  it('patches inactive node and observer lists even when their routes are unmounted', () => {
    ...
    const nodeKey = nodeQueries.list({
      regionKey: 'YOW',
      iatas: ['YOW'],
      sort: 'neighbors',
    }).queryKey;
    const observerKey = observerQueries.list({
      regionKey: 'YOW',
      iatas: ['YOW'],
      sort: 'last_seen',
    }).queryKey;
    ...
    const updatedNode =
      client.getQueryData<InfiniteData<CursorPage<NodeSummary>>>(nodeKey)?.pages[0]?.items[0];
    const updatedObserver =
      client.getQueryData<InfiniteData<CursorPage<ObserverSummary>>>(observerKey)?.pages[0]
        ?.items[0];
  ...
  it('invalidates server-sorted or filtered lists when a direct patch could break membership/order', () => {
    ...
    expect(
      client.getQueryData<InfiniteData<CursorPage<NodeSummary>>>(nodeKey)?.pages[0]?.items[0]?.name,
    ).toBe('Alpha');
    expect(
      client.getQueryData<InfiniteData<CursorPage<ObserverSummary>>>(observerKey)?.pages[0]
        ?.items[0]?.status,
  ...
  it('reconnect healing refetches only one fresh packet page instead of replaying cached cursors', async () => {
    ...
    await waitFor(() => {
      expect(
        client.getQueryData<InfiniteData<CursorPage<PacketSummary>>>(queryKey)?.pages,
      ).toHaveLength(1);
    });
  });
});
