...
vi.mock('../../src/api/ws-instance', () => ({
  wsManager: {
    onPacketObservation: (callback: (data: unknown) => void) =>
      subscribe(listeners.packet, callback),
    ...
    onObserverStatus: (callback: (data: unknown) => void) =>
      subscribe(listeners.observer, callback),
    onNodeUpdate: (callback: (data: WsNodeUpdate['data']) => void) =>
      subscribe(listeners.node, callback),
  },
}));
...
describe('QueryWsBridge listener lifecycle', () => {
  it('keeps exactly one global listener and synchronizes cache without a feature route mounted', () => {
    ...
    expect(
      client.getQueryData<InfiniteData<CursorPage<NodeSummary>>>(key)?.pages[0]?.items[0]?.lat,
    ).toBe(46);
