...
describe('useInfinitePages', () => {
  ...
  it('with auto:false, loads only the first page until loadMore is called', async () => {
    ...
    const { result } = renderHook(
      () =>
        useInfinitePages<Row>({
          options: optionsFor(['t', 'ondemand'], queryFn),
          getId: rowId,
          auto: false,
        }),
      { wrapper: wrapper() },
  ...
  it('keeps previous items (no skeleton) while a new key loads when keepPrevious is set', async () => {
    ...
    const { result, rerender } = renderHook(
      ({ k }: { k: string }) =>
        useInfinitePages<Row>({
          options: optionsFor(['kp', k], queryFn),
          getId: rowId,
          keepPrevious: true,
        }),
    ...
    queryFn.mockImplementationOnce(
      () =>
        new Promise<{ items: Row[]; nextCursor: number | null; hasMore: boolean }>((res) => {
          resolveB = res;
        }),
    );
    rerender({ k: 'B' });
