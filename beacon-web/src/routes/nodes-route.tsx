...
function nodeViewState(search: Record<string, unknown>): NodeTableViewState {
  return {
    ...
    pathsFilter: (search.np === 'true' || search.np === 'false'
      ? search.np
      : '') as MultibyteFilter,
    tracesFilter: (search.ntr === 'true' || search.ntr === 'false'
      ? search.ntr
      : '') as MultibyteFilter,
...
export function NodesRoute() {
  ...
  const onViewStateChange = useCallback(
    (patch: Partial<NodeTableViewState>, options?: { replace?: boolean }) => {
      navigate({
        to: '.',
        ...
        search: (prev) => {
          ...
          return {
            ...
            ...(patch.searchField !== undefined
              ? {
                  nsf: (patch.searchField === 'name' ? undefined : patch.searchField) as
                    'pubkey' | undefined,
                }
              : {}),
