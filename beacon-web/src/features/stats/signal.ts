...
export function signalHistogramOption(
  metric: SignalMetric | undefined,
  name: string,
  unit: string,
  c: ChartColors,
  ...
  return {
    ...
    aria: {
      enabled: true,
      label: {
        description: t('charts.histogramDescription', { name, unit }),
      },
    },
    ...
    xAxis: {
      type: 'category',
      data: bins.map((bin) => `${signalBinLabel(bin)} ${unit}`),
      axisLabel: { color: c.textMuted, fontSize: 9, rotate: 35 },
...
export function signalTrendOption(
  hours: ReturnType<typeof signalHours>,
  metric: 'snr' | 'rssi',
  c: ChartColors,
  t: TFunction = defaultT,
): EChartsOption {
  ...
  return {
    ...
    aria: {
      enabled: true,
      label: {
        description: t('charts.meanDescription', { name: metric.toUpperCase(), unit }),
      },
    },
    ...
    tooltip: {
      ...
      valueFormatter: (value) =>
        typeof value === 'number' ? `${value.toFixed(2)} ${unit}` : 'No sample',
    },
    xAxis: {
      type: 'time',
    ...
    series: [
      {
        ...
        itemStyle: { color },
        areaStyle: { color: withAlpha(color, 0.08) },
        data: hours.map((hour) => [
          hour.hour,
          hour[metric === 'snr' ? 'snrAverage' : 'rssiAverage'],
        ]),
      },
    ],
  };
...
export function signalCoverageOption(
  data: SignalStats | undefined,
  c: ChartColors,
  t: TFunction = defaultT,
): EChartsOption {
  return {
    ...
    aria: {
      enabled: true,
      label: {
        description: t('charts.signalDescription'),
      },
    },
    ...
    yAxis: {
      type: 'category',
      data: ['SNR', 'RSSI'],
      axisLabel: { color: c.textNormal },
      axisLine: { show: false },
      axisTick: { show: false },
    },
    series: [
      {
        name: t('charts.available'),
        type: 'bar',
        stack: 'samples',
        barMaxWidth: 22,
