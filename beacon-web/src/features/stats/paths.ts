...
export function pathHours(data: PathStats | undefined) {
  ...
  return Array.from({ length: Math.min(721, Math.ceil((data.until - first) / HOUR)) }, (_, i) => {
    ...
    return {
      hour,
      oneByte: row?.oneByte ?? null,
      twoByte: row?.twoByte ?? null,
      threeByte: row?.threeByte ?? null,
    };
  });
}

export function pathLengthOption(
  bins: PathLengthBin[],
  c: ChartColors,
  ...
  return {
    ...
    aria: {
      enabled: true,
      label: {
        description: t('charts.pathLengthDescription'),
      },
    },
    ...
    series: [
      {
        ...
        data: entries.map((i) => ({
          ...
          itemStyle: {
            color: i === 0 ? c.textDim : blend(c.primary, c.secondary, i / Math.max(1, max)),
...
export function pathTrendOption(
  hours: ReturnType<typeof pathHours>,
  c: ChartColors,
  t: TFunction = defaultT,
): EChartsOption {
  return {
    ...
    aria: {
      enabled: true,
      label: {
        description: t('charts.pathsDescription'),
      },
    },
    legend: {
      top: 0,
    ...
    series: (['oneByte', 'twoByte', 'threeByte'] as const).map((field, i) => ({
      ...
      showSymbol: true,
      symbolSize: 4,
      lineStyle: { width: 1.5, color: c.series[i] },
      itemStyle: { color: c.series[i] },
      areaStyle: { color: withAlpha(c.series[i]!, 0.32) },
