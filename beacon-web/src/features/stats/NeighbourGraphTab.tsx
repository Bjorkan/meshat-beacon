...
export function NeighbourGraphTab() {
  ...
  const option = useMemo(
    () => neighbourGraphOption(ego ?? graph, colors, { ego: !!ego, t }),
    [ego, graph, colors, t],
  );
    ...
    <div className="flex h-full min-h-0 flex-col">
      {ego ? (
        <div className="shrink-0 border-b border-border bg-bg-surface px-4 py-2 text-center text-xs font-mono text-text-muted">
          {t('stats.neighbourhood', {
            name: selectedNode?.name ?? selectedId,
            count: ego.nodes.length - 1,
          })}
        </div>
      ) : (
        <div className="flex shrink-0 items-center gap-3 border-b border-border bg-bg-surface px-4 py-2">
          <span className="text-xs font-mono text-text-muted">
            {graph.capped
              ? t('stats.showingNodes', { shown: CAP, total: graph.total })
              : t('stats.nodeCount', { count: graph.total })}
          </span>
