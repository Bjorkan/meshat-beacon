...
export function trafficModel(
  ...
  for (const point of points) {
    if (
      ...
      point.hour < start ||
      point.hour > end ||
      !Number.isFinite(point.observationCount) ||
      point.observationCount < 0
    )
      continue;
    const hour = Math.floor(point.hour / HOUR) * HOUR;
    const name = point.iata.trim().toUpperCase() || t('charts.unassigned');
  ...
  const series = names.map((name) => ({
    ...
    values: hours.map(({ hour }) =>
      buckets.has(hour) ? (buckets.get(hour)?.get(name) ?? 0) : null,
    ),
  }));
  if (areas.length > names.length) {
    series.push({
      name: t('charts.otherIatas'),
      total: areas.slice(7).reduce((sum, area) => sum + area.total, 0),
      values: hours.map(({ hour, total }) =>
        total === null
          ? null
          : total - names.reduce((sum, name) => sum + (buckets.get(hour)?.get(name) ?? 0), 0),
      ),
    });
  }
  ...
  for (const { hour, total } of hours) {
    ...
    heatmap.push([
      new Date(hour).getUTCHours(),
      days.indexOf(new Date(hour).toISOString().slice(0, 10)),
      total,
    ]);
  ...
  return {
    ...
    areas,
    series,
    reportedHours: buckets.size,
    total: areas.reduce((sum, area) => sum + area.total, 0),
  };
}

export type TrafficModel = ReturnType<typeof trafficModel>;

export function trafficTrendOption(
  model: TrafficModel,
  c: ChartColors,
  t: TFunction = defaultT,
): EChartsOption {
  ...
  return {
    ...
    aria: {
      enabled: true,
      label: {
        description: t('charts.trafficDescription'),
      },
    },
    ...
    legend: {
      ...
      left: 0,
      right: 0,
      textStyle: { color: c.textNormal, fontSize: 10 },
      inactiveColor: c.textDim,
    },
    xAxis: {
      type: 'time',
      min: first === last && first !== undefined ? first - HOUR / 2 : first,
      max: first === last && last !== undefined ? last + HOUR / 2 : last,
      axisLabel: { color: c.textMuted, fontSize: 10, hideOverlap: true },
    ...
    series: model.series.map((series, index) => ({
      ...
      symbol: 'circle',
      symbolSize: 5,
      showSymbol: present.length <= 2,
      data: model.hours.map((hour, i) => [hour.hour, series.values[i]]),
      lineStyle: { color: c.series[index], width: 1.5 },
      itemStyle: { color: c.series[index] },
      areaStyle: { color: c.series[index], opacity: 0.45 },
    })),
  };
}

export function trafficHeatmapOption(
  model: TrafficModel,
  c: ChartColors,
  t: TFunction = defaultT,
): EChartsOption {
  return {
    ...
    xAxis: {
      ...
      axisTick: { show: false },
      axisLine: { show: false },
      splitArea: { show: true, areaStyle: { color: [c.bgRaised, c.bgSurface] } },
    },
    yAxis: {
      type: 'category',
      data: model.days,
    ...
    visualMap: {
      min: 0,
      max: Math.max(1, model.peak?.total ?? 0),
      calculable: false,
      orient: 'horizontal',
      left: 'center',
      bottom: 0,
    ...
    series: [
      {
        name: t('charts.receptions'),
        type: 'heatmap',
        data: model.heatmap,
        itemStyle: { borderColor: c.bgSurface, borderWidth: 1 },
        emphasis: { itemStyle: { borderColor: c.textBright, borderWidth: 1 } },
      },
    ],
  };
}
