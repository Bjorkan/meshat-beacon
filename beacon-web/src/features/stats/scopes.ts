...
export function scopeChartOption(
  rows: ScopeStats[],
  metric: ScopeMetric,
  colors: ChartColors,
  ...
  const ranked = rows
    .map((row) => ({
      name: row.name,
      value: row[metric],
      color: colors.series[(indices.get(row.name) ?? 0) % colors.series.length] ?? colors.primary,
    }))
    .sort((a, b) => b.value - a.value || a.name.localeCompare(b.name));
  const shown = ranked.slice(0, 12);
  if (ranked.length > 12)
    shown.push({
      name: t('charts.otherScopes'),
      value: ranked.slice(12).reduce((sum, row) => sum + row.value, 0),
      color: colors.textDim,
    });
  return {
    ...leaderboardOption(shown, colors, 126),
    tooltip: { trigger: 'item', renderMode: 'richText', ...tooltipStyle(colors) },
    aria: {
      enabled: true,
      label: {
        description: t('charts.scopeDescription'),
      },
    },
  };
}
