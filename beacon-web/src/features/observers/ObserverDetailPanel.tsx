...
export function ObserverDetailPanel({
  ...
  return (
    <DetailPanel
      ...
      headerAction={
        <CopyLinkButton
          to={`/observers/${encodeURIComponent(observerId)}`}
          params={{}}
          ariaLabel={t('observers.copyLink')}
        />
      }
      isLoading={isLoading}
      notFound={!observer}
        ...
        <>
            ...
            <Section title={t('details.firmware')}>
              <div className="flex flex-col gap-0.5 font-mono text-[13px]">
                ...
                {observer.softwareVersion && (
                  <Field label={t('details.software')} value={observer.softwareVersion} />
                )}
                {observer.hardwareModel && (
                  <Field label={t('details.hardware')} value={observer.hardwareModel} />
                )}
              </div>
            </Section>
          )}
          ...
          <Section title={t('entities.status')}>
            <div className="flex flex-wrap gap-x-4 gap-y-0.5 font-mono text-[13px]">
              ...
              {observer.uptimeSeconds != null && (
                <Field label={t('details.uptime')} value={formatUptime(observer.uptimeSeconds)} />
              )}
              {stats?.queue_len != null && (
                <Field label={t('details.queue')} value={stats.queue_len} />
              )}
            </div>
            ...
            (stats.rx_air_secs != null ||
              ...
              <Section title={t('details.airtime')}>
                <div className="flex flex-wrap gap-x-4 gap-y-0.5 font-mono text-[13px]">
                  ...
                  {stats.tx_air_secs != null && (
                    <Field label="TX" value={formatAirtime(stats.tx_air_secs)} />
                  )}
                </div>
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-0.5 font-mono text-[13px] mt-1">
                    {stats.recv_errors != null && (
                      <Field
                        ...
                        value={stats.recv_errors.toLocaleString()}
                      />
                    )}
                    {stats.errors != null && (
                      <Field label={t('details.errors')} value={stats.errors.toLocaleString()} />
                    )}
                  </div>
                )}
              </Section>
                    ...
                    return (
                      <div key={b.name} className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-3 font-mono text-[13px]">
                          ...
                          />
                          <Field
                            label={t('details.packet')}
                            value={b.lastPacketAt ? <Timestamp value={b.lastPacketAt} /> : '—'}
                          />
                        </div>
                      </div>
                    );
