...
export function PacketAnalyzerDrawer({
  ...
  const fieldRanges = detail ? computeFieldRanges(detail, selectedObs, totalBytes) : {};

  const headerHex = rawHex.slice(0, 2);

  return (
    <div
      data-testid="packet-analyzer-drawer"
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-border-subtle shrink-0">
        ...
        <div className="flex items-center gap-1.5">
          {detail && (
            <CopyLinkButton
              to="/packets"
              params={{ hash: detail.packetHash, analyze: '1' }}
              ariaLabel={t('packets.copyLink')}
            />
          )}
        ...
        {!detail ? (
          ...
          <>
            <DrawerSection title={t('details.summary')} first>
              <div className="flex items-center gap-2 mb-2">
                ...
                <Badge variant={payloadTypeVariant(detail.header.payloadType)}>
                  {PAYLOAD_TYPE_NAMES[detail.header.payloadType as PayloadTypeValue] ??
                    t('packets.unknown')}
                </Badge>
              ...
              <div className="flex items-center gap-3 text-[13px] font-mono">
                ...
                <span>
                  <span className="text-text-dim">{t('packets.propagation')} </span>
                  <span className="text-text-normal">
                    {formatPropagation(detail.firstToLastMs)}
                  </span>
                </span>
              </div>
            </DrawerSection>
            ...
            <div className="px-3 py-2 border-b border-border-subtle">
              ...
              >
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" aria-hidden>
                  <path
                    d="M9 5l-6 2v12l6-2 6 2 6-2V5l-6 2-6-2z"
                    stroke="currentColor"
                    strokeWidth="1.4"
            ...
            {detail.observations.length >= 1 && (
              <DrawerSection
                title={t('packets.observationsCount', { count: detail.observations.length })}
                collapsible
                defaultOpen={false}
              >
              ...
              <div className="flex flex-col gap-2.5 font-mono text-[13px]">
                ...
                <ColorAccentField field="header">
                  <div className="text-text-dim text-xs font-medium uppercase tracking-wider mb-1">
                    {t('packets.headerByte')}
                  </div>
                  <div className="flex gap-x-4">
                    <span>
                      <span className="text-text-dim">Ver </span>
                      <span className="text-text-normal">{detail.header.payloadVersion}</span>
                    </span>
                    <span>
                    ...
                    <span>
                      ...
                      <span className="text-text-normal">
                        {ROUTE_TYPE_NAMES[detail.header.routeType as RouteTypeValue] ?? '?'} (
                        {detail.header.routeType})
                      </span>
                    </span>
                  </div>
                  {headerHex && <HeaderBitBreakdown headerHex={headerHex} />}
                </ColorAccentField>
                ...
                {detail.transportCodes && (
                  <ColorAccentField field="transport">
                    ...
                    <span className="text-text-normal">
                      {detail.transportCodes.regionCode} / {detail.transportCodes.subRegionCode}
                    </span>
                  </ColorAccentField>
                )}
                ...
                {selectedObs && (
                  <ColorAccentField field="pathLength">
                    <div className="text-text-dim text-xs font-medium uppercase tracking-wider mb-1">
                      {t('packets.pathLength')}
                    </div>
                    <div className="flex gap-x-4">
                      <span>
                        <span className="text-text-dim">{t('packets.hashSize')} </span>
                        <span className="text-text-normal">{selectedObs.pathLength.hashSize}B</span>
                      </span>
                      <span>
                        <span className="text-text-dim">{t('packets.hops')} </span>
                        <span className="text-text-normal">{selectedObs.pathLength.hopCount}</span>
                      </span>
                    </div>
                    <PathLengthBitBreakdown
                      pathLengthByte={parseInt(selectedObs.pathLength.raw, 16)}
                    />
                  </ColorAccentField>
                )}
                ...
                {selectedObs?.pathBytes && (
                  <ColorAccentField field="pathData">
                    ...
                    <PathData
                      pathBytes={selectedObs.pathBytes}
                      hashSize={selectedObs.pathLength.hashSize}
                      resolvedPath={selectedObs.resolvedPath}
                      onViewNode={onViewNode}
                    />
                  </ColorAccentField>
                )}
                ...
                <DrawerSection title={t('packets.payloadBreakdown')}>
                  <div className="font-mono text-[13px]">
                    <PayloadBreakdown
                      ...
                      resolvedSource={selectedObs?.resolvedSource}
                      resolvedDestination={selectedObs?.resolvedDestination}
                      onViewNode={onViewNode}
                    />
                  </div>
                </DrawerSection>
            ...
            {detail.parsedPayload &&
              typeof detail.parsedPayload === 'string' &&
              (() => {
