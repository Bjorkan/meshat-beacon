  ...
  if (offset < totalBytes) {
    ...
    if (pp && typeof pp === 'object') {
      ...
      if (detail.header.payloadType === PayloadType.ADVERT && typeof pp.publicKey === 'string') {
        ...
        detail.header.payloadType === PayloadType.ANON_REQ &&
        typeof pp.ephemeralPubKey === 'string'
      ) {
...
export function PathLengthBitBreakdown({ pathLengthByte }: { pathLengthByte: number }) {
  ...
  return (
    <div className="mt-1.5 font-mono text-xs bg-bg-base border border-primary/20 rounded px-2 py-1.5">
      <div className="flex items-start gap-1">
        <span className="text-text-dim leading-none pt-px">
          0x{pathLengthByte.toString(16).toUpperCase().padStart(2, '0')} = [
        </span>
        <span className="flex flex-col items-center leading-none">
...
export function AdvertFlagsBitBreakdown({ flagsByte }: { flagsByte: number }) {
  ...
  return (
    <div className="mt-1.5 font-mono text-xs bg-bg-base border border-primary/20 rounded px-2 py-1.5">
      <div className="flex items-start gap-1">
        <span className="text-text-dim leading-none pt-px">
          0x{flagsByte.toString(16).toUpperCase().padStart(2, '0')} = [
        </span>
        <span className="flex flex-col items-center leading-none">
...
export function ColorAccentField({
  ...
  return (
    <div
      className={`pl-2 -mx-1 py-0.5 -my-0.5 border-l-2 ${FIELD_COLORS[field].accent} ${className ?? ''}`}
    >
      {children}
    </div>
  );
  ...
  return (
    <div className="flex flex-col gap-1.5 font-mono text-[13px]">
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-0.5 text-[13px]">
        ...
        <span>
          <span className="text-text-dim">RSSI </span>
          <span className={sigClass}>{observation.rssi ?? '—'}</span>
        </span>
        <span>
          <span className="text-text-dim">Prop </span>
        ...
        <span>
          ...
          <span className="text-text-normal">{observation.pathLength.hopCount}</span>
        </span>
      </div>
