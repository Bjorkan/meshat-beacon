...
export type PathChip =
  ...
  | {
      kind: 'hex';
      label: string;
      confidence: Exclude<PathConfidence, 'high'>;
...
function rawHops(pathBytes: string | undefined, hashSize: number, hopCount: number): string[] {
  ...
  for (let i = 0; i < pathBytes.length; i += width)
    out.push(pathBytes.slice(i, i + width).toUpperCase());
  return out;
}
...
export function buildPathSummary(packet: PacketSummary): PathSummary {
  ...
  const chips = raw
    .slice(0, MAX_PATH_HOPS_SHOWN)
    .map((hash, i) => chipFor(hash, observer.resolvedPath?.[i]));
