    ...
    <div className="bg-bg-base border border-border rounded px-3 py-2 flex flex-col gap-1.5">
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="font-mono text-[11px] text-text-dim">
          {t('packets.hop', { count: route.totalHops })}
        </span>
      </div>
      <div className="flex flex-wrap items-center gap-1 font-mono text-[13px]">
    ...
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="font-mono text-[11px] text-text-dim">
          {t('routes.summary', {
            hops: r.hopCount,
            observations: r.observationCount.toLocaleString(),
          })}
        </span>
      </div>
      ...
      <div className="flex items-center gap-2 font-mono text-[11px] text-text-muted">
        <span>
          {t('common.first')} <Timestamp value={r.firstSeen} />
        </span>
        <span aria-hidden>·</span>
        <span>
          {t('common.last')} <Timestamp value={r.lastSeen} />
        </span>
      </div>
    </div>
  );
...
export function RouteTable() {
  ...
  const { data: searchRoutes, isLoading: searchLoading } = useQuery({
    ...routeQueries.search(
      search ? { iata: search.iatas[0]!, from: search.from, to: search.to } : null,
    ),
  ...
  const { data: crossRoutes, isLoading: crossLoading } = useQuery(
    routeQueries.cross(
      search && isCross ? { iatas: search.iatas, fromHash: search.from, toHash: search.to } : null,
    ),
  );
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 min-w-0 w-full">
      ...
      <div className="flex flex-col lg:flex-row lg:flex-wrap lg:items-center gap-1.5 gap-y-1.5 px-4 py-2 border-b border-border-subtle bg-bg-base shrink-0">
        <div className="flex items-center gap-1.5">
          <span className="text-text-muted text-[11px] uppercase tracking-wider mr-1 shrink-0">
            {t('routes.findPath')}
          </span>
          <input
            ...
            onKeyDown={onKeyDown}
          />
          <span className="text-text-dim text-xs shrink-0" aria-hidden>
            →
          </span>
          <input
            className={`${inputClass} flex-1 min-w-0 lg:flex-none lg:w-24`}
