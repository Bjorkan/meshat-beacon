...
import {
  ...
  upsertNodePages,
} from '../features/nodes/node-updates';
import {
...
function nodeCoordinatesChanged(
  ...
  return (
    (data.lat !== undefined && data.lat !== previous.lat) ||
...
export function syncNodeUpdate(queryClient: QueryClient, data: WsNodeUpdate['data']): void {
  ...
  for (const [, neighbors] of queryClient.getQueriesData<
    Array<{ id: string; lat?: number; lng?: number }>
  >({ queryKey: ['node-neighbors'] })) {
    coordinatesChanged ||= nodeCoordinatesChanged(
      neighbors?.find((node) => node.id === data.nodeId),
      data,
    );
  }
  ...
  >({ queryKey: nodeQueries.all() })) {
    ...
    const context = {
      ...
      sort: (String(queryKey[8] ?? 'name') || 'name') as
        'name' | 'type' | 'radio' | 'neighbors' | 'last_seen',
      iatas: regionIatas,
    };
...
export function syncObserverStatus(queryClient: QueryClient, data: WsObserverStatus['data']): void {
  ...
  >({ queryKey: observerQueries.all() })) {
    ...
    const context = {
      ...
      scope: String(queryKey[7] ?? '') || undefined,
      sort: (String(queryKey[8] ?? 'name') || 'name') as
        'name' | 'type' | 'radio' | 'iata' | 'status' | 'last_seen',
      iatas: regionIatas,
    };
  ...
  void queryClient.invalidateQueries({
    queryKey: ['observer-telemetry', data.observerId],
    refetchType: 'active',
  });
}
...
export function syncChannelMessage(
  ...
  queryClient.setQueryData<InfiniteData<CursorPage<ChannelMessage>>>(messageKey, (old) => {
    if (
      !old ||
      old.pages.some((page) => page.items.some((message) => message.packetHash === data.packetHash))
    )
      return old;
    const pages = old.pages.map((page, index) =>
      index === 0 ? { ...page, items: [...page.items, data] } : page,
    );
    return { ...old, pages };
  });
}

export function syncPacketObservation(
  queryClient: QueryClient,
