...
if (schema.swagger !== '2.0')
  throw new Error(`Expected Swagger 2.0, got ${schema.swagger ?? 'unknown'}`);
...
function tsType(node = {}, refPrefix = '') {
  ...
  if (node.type === 'object' || node.properties || node.additionalProperties) {
    if (node.additionalProperties && !node.properties)
      return `Record<string, ${tsType(node.additionalProperties, refPrefix)}> `.trim();
    ...
    if (node.additionalProperties)
      props.push(`[key: string]: ${tsType(node.additionalProperties, refPrefix)};`);
    return `{ ${props.join(' ')} }`;
  }
  return 'unknown';
}
...
for (const [key, def] of Object.entries(schema.definitions ?? {})) {
  ...
  const description = def.description
    ? `/** ${String(def.description).replace(/\*\//g, '*\\/')} */\n`
    : '';
...
for (const { route, method, parameters, name, op } of operations) {
  ...
  const fields = [...required, ...optional].map(
    (p) => `${p.name}${p.required ? '' : '?'}: ${tsType(p.schema ?? p, 'Models.')};`,
  );
  if (bodyParam)
    fields.push(
      `${bodyParam.name || 'body'}${bodyParam.required ? '' : '?'}: ${tsType(bodyParam.schema ?? {}, 'Models.')};`,
    );
  ...
  for (const p of pathParams)
    routeExpr = routeExpr.replace(
      `{${p.name}}`,
      `\${encodeURIComponent(String(params.${p.name}))}`,
    );
  if (pathParams.length) routeExpr = '`' + routeExpr.slice(1, -1).replace(/\\\$\{/g, '${') + '`';
  const queryEntries = queryParams
    .map((p) => `${JSON.stringify(p.name)}: params${fields.length ? '' : '?'}.${p.name}`)
    .join(', ');
